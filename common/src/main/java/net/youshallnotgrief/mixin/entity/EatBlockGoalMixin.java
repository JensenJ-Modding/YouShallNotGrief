...
public class EatBlockGoalMixin {
    ...
    private boolean youshallnotgrief$logAnimalEatGrass(
        ...
        return BlockUtils.wrapLevelRemoveBlock(level, pos, b, original, oldState -> {
            if (ServerConfig.logMobEatBlock.get()) {
                BlockUtils.addToDatabase(
                        pos, level, oldState, Blocks.AIR.defaultBlockState(), BlockSetCauses.ATE, mob, "");
            }
        });
    }
    ...
    private boolean youshallnotgrief$logAnimalEatBlock(
        ...
        return BlockUtils.wrapLevelSetBlock(level, pos, state, i, original, oldState -> {
            if (ServerConfig.logMobEatBlock.get()) {
                BlockUtils.addToDatabase(
                        pos, level, oldState, Blocks.DIRT.defaultBlockState(), BlockSetCauses.ATE, mob, "");
            }
        });
    }
}
