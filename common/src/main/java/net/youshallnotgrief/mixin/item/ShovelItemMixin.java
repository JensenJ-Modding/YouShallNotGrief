...
public abstract class ShovelItemMixin {
    ...
    public boolean youshallnotgrief$logPathFlattening(
            Level level,
            BlockPos pos,
            BlockState state,
            int i,
            ...
            if (oldState.getBlock() == Blocks.CAMPFIRE) {
                if (ServerConfig.logExtinguish.get()) {
                    BlockUtils.addToDatabase(
                            pos, level, oldState, state, BlockSetCauses.EXTINGUISH, context.getPlayer(), "");
                }
            } else {
                if (ServerConfig.logPaving.get()) {
                    BlockUtils.addToDatabase(
                            pos, level, oldState, state, BlockSetCauses.PAVED, context.getPlayer(), "");
                }
            }
        });
    }
