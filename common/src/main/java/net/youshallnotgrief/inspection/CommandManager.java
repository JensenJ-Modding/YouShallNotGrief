...
public class CommandManager {
        ...
        for (String alias : aliases) {
            dispatcher.register(Commands.literal(alias)
                    .requires((commandSourceStack) ->
                            commandSourceStack.hasPermission(ServerConfig.inspectionOpNeeded.get() ? 1 : 0))
    ...
    private static int toggleInspectModeOther(CommandContext<CommandSourceStack> context)
                ...
                if (InspectionMode.isPlayerInspecting(player)) {
                    stack.sendSuccess(
                            () -> Component.translatable(
                                            "msg.youshallnotgrief.inspection.disable.other", player.getName())
                                    .withStyle(style -> style.withColor(MiscUtils.getTextColourFromConfig(
                                            ServerConfig.inspectionPrimaryColour.get()))),
                            true);
                } else {
                    stack.sendSuccess(
                            () -> Component.translatable(
                                            "msg.youshallnotgrief.inspection.enable.other", player.getName())
                                    .withStyle(style -> style.withColor(MiscUtils.getTextColourFromConfig(
                                            ServerConfig.inspectionPrimaryColour.get()))),
                            true);
                }
