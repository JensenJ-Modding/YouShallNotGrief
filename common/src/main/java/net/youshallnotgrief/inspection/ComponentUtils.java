...
public class ComponentUtils {
    ...
    public static MutableComponent formatTimeAgo(Timestamp timestamp) {
        ...
        if (days >= 1) {
            return Component.literal(String.format(timeFormat, days))
                    .append(Component.translatable("msg.youshallnotgrief.inspection.time.days"));
        } else if (hours >= 1) {
            return Component.literal(String.format(timeFormat, hours))
                    .append(Component.translatable("msg.youshallnotgrief.inspection.time.hours"));
        } else {
            return Component.literal(String.format(timeFormat, minutes))
                    .append(Component.translatable("msg.youshallnotgrief.inspection.time.minutes"));
        }
    }
    ...
    public static MutableComponent getItemComponentFromString(String itemName, int amount) {
        ...
        comp.append(Component.literal(Math.abs(amount) + " ")
                .withStyle(style -> style.withColor(
                        MiscUtils.getTextColourFromConfig(ServerConfig.inspectionPrimaryColour.get()))));
        if (item == null) {
            ...
            return comp.append(
                    Component.literal(InventoryUtils.getItemName(item)).withStyle(style -> style.withColor(
                                    MiscUtils.getTextColourFromConfig(ServerConfig.inspectionPrimaryColour.get()))
    ...
    public static MutableComponent getSourceComponentFromString(String source, String sourceDesc) {
        if (sourceDesc.isEmpty()) {
            ...
            return Component.literal(source).withStyle(style -> style.withColor(
                            MiscUtils.getTextColourFromConfig(ServerConfig.inspectionPrimaryColour.get()))
    ...
    public static MutableComponent formatMobSourceDesc(String sourceDesc) {
        ...
        MutableComponent comp =
                Component.translatable("msg.youshallnotgrief.inspection.block.cause.mob.source", sourceEntityName);
        for (int i = 2; i < elements.size(); i++) {
            String targetEntityName = formatEntityNameFromDatabase(elements.get(i));
            comp.append("\n")
                    .append(Component.translatable(
                            "msg.youshallnotgrief.inspection.block.cause.mob.target",
                            sourceEntityName,
                            targetEntityName));
        }
        return comp;
