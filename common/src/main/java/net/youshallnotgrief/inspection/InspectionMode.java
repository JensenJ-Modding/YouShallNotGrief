...
public class InspectionMode {
    ...
    public static void registerEvents() {
        ...
        PlayerEvent.CHANGE_DIMENSION.register(
                (ServerPlayer player, ResourceKey<Level> fromDim, ResourceKey<Level> toDim) -> {
                    CURRENTLY_SELECTED_BLOCK.remove(player);
                    CURRENTLY_SELECTED_ENTITY.remove(player);
                });
    ...
    public static void enableInspectMode(Player player) {
        ...
        player.sendSystemMessage(Component.translatable("msg.youshallnotgrief.inspection.enable")
    ...
    public static void disableInspectMode(Player player) {
        ...
        player.sendSystemMessage(Component.translatable("msg.youshallnotgrief.inspection.disable")
                .withStyle(style -> style.withColor(
                        MiscUtils.getTextColourFromConfig(ServerConfig.inspectionPrimaryColour.get()))));
    }
    ...
    public static void showDetails(Player player, int pageNumber) {
        if (!InspectionMode.INSPECTING_PLAYERS.contains(player)) {
            player.sendSystemMessage(Component.translatable("error.youshallnotgrief.inspection.outofinspection")
                    .withStyle(style -> style.withColor(
        ...
        player.sendSystemMessage(Component.translatable("error.youshallnotgrief.inspection.inspectfirst")
                .withStyle(style ->
                        style.withColor(MiscUtils.getTextColourFromConfig(ServerConfig.inspectionErrorColour.get()))));
    }
    ...
    public static void getDataForBlock(
            BlockPos pos,
            String dimensionID,
            int pageNumber,
            Player player,
            Consumer<RetrieveResult<CombinedBlockData>> consumer) {
        DatabaseBlockQueryManager.retrieveFromDatabase(
                new CombinedBlockData(pos, dimensionID),
                ACTIONS_PER_PAGE,
                pageNumber * ACTIONS_PER_PAGE,
                (isBusy, data) -> {
                    if (isBusy) {
                        player.sendSystemMessage(Component.translatable("error.youshallnotgrief.inspection.busy")
                                .withStyle(style -> style.withColor(
                                        MiscUtils.getTextColourFromConfig(ServerConfig.inspectionErrorColour.get()))));
                    } else if (data == null) {
                        player.sendSystemMessage(Component.translatable("error.youshallnotgrief.inspection.database")
                                .withStyle(style -> style.withColor(
                                        MiscUtils.getTextColourFromConfig(ServerConfig.inspectionErrorColour.get()))));
                        YouShallNotGriefMod.LOGGER.error(
                                "{} failed to inspect page {} of block at coordinates {} {} {} in {}. The database failed to retrieve data for this location.",
                                EntityUtils.getEntityCustomNameOrFallbackID(player),
                                pageNumber + 1,
                                pos.getX(),
                                pos.getY(),
                                pos.getZ(),
    ...
    public static void getDataForEntity(
            Entity entity,
            BlockPos pos,
            String dimensionID,
            int pageNumber,
            Player player,
            Consumer<RetrieveResult<EntityItemTransactionData>> consumer) {
        DatabaseManager.ENTITY_ITEM_TRANSACTION_DATA_MANAGER.retrieveFromDatabase(
                new EntityItemTransactionData(entity.getStringUUID()),
                ACTIONS_PER_PAGE,
                pageNumber * ACTIONS_PER_PAGE,
                (data) -> {
                    if (data == null) {
                        player.sendSystemMessage(Component.translatable("error.youshallnotgrief.inspection.database")
                                .withStyle(style -> style.withColor(
                                        MiscUtils.getTextColourFromConfig(ServerConfig.inspectionErrorColour.get()))));
                        YouShallNotGriefMod.LOGGER.error(
                                "{} failed to inspect page {} of entity {} at coordinates {} {} {} in {}. The database failed to retrieve data for this entity.",
                                EntityUtils.getEntityCustomNameOrFallbackID(player),
                                pageNumber + 1,
